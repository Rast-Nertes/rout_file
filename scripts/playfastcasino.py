...
async def login(driver):
    ...
    try:
        await input_data(driver, 60, '//input[@name="email"]', user_email)
        await input_data(driver, 30, '//input[@name="password"]', user_password)
        await click(driver, 30, '//*[@id="root"]/div[1]/div/div[2]/div/div[2]/div[1]/div[2]/form/div[2]/button')
    except Exception as e:
        return {"status": "0", "ext": f"error login \n{e}"}

    await asyncio.sleep(5)
    await driver.get('https://playfastcasino.com/account/financials/deposit')

    try:
        src_payment = await driver.find_element(By.XPATH, '//iframe[@title="MyAccount"]', timeout=40)
        src = await src_payment.get_attribute('src')

        await driver.get(src)
    except Exception as e:
        return {"status":"0", "ext":f"error scr payment \n{e}"}
...
async def get_wallet():
    async with webdriver.Chrome(options=options) as driver:
        ...
        try:
            ...
            return {
                "address": address,
                "amount": amount.replace("USDT", '').replace("\xa0", '').replace(" ", '').replace("$", ''),
                "currency": "usdt"
            }
        except Exception as e:
