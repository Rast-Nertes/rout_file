...
def login(driver):
    ...
    sleep(2.5)
    driver.get('https://usdfarm.top/user/insert/insertusdt')
...
def get_wallet():
    with webdriver.Chrome(options=options, seleniumwire_options=proxy_options) as driver:
        ...
        sleep(4.5)
        try:
            wait_visibility(driver, 60, '//img[@class="qrcode"]')
            address_elem = driver.find_element(By.XPATH, '//img[@class="qrcode"]')
            src = address_elem.get_attribute('alt')
            address = src.split("?")[0].split(":")[1]

            amount = src.split("value=")[1]
